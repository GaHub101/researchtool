...
**20 Tabellen, rund 250 Felder.** Alles, was nicht mit ⊕ markiert ist, entsteht
automatisch beim Import der Dateien aus `seed/` — dort steht dann nur der Typ auf
*Text* und muss korrigiert werden.
...
- Jedes `__pk…` — Text, Automatisch eingeben ▸ Berechneter Wert `Get ( UUID )`,
...
| `s_AktuelleSectionID` | T (für Portalfilter F-20.5) |
...
> Die drei globalen Felder werden beim Start von Skript S-01 gefüllt und steuern
> Portalfilter und Sichtbarkeiten. Sie liegen bewusst in `Projects` und nicht in
> `zz_Utility` — dann sind sie ohne zusätzlichen Bezug überall erreichbar.
...
| `Nachname` `Vorname` | T | |
| `Kontoname` | T | verbindet die Person mit dem FileMaker-Konto |
| `Mail` | T | |
| `Stufe` | T | WL-11 |
| `Aktiv` | Z | Vorgabe 1 |
| `c_Name` | ⊕ T | F-17 |
...
`__pkRoleID` T · `Bezeichnung` T · `Reihenfolge` Z

Startdaten in `seed/Roles.tab`: PI / Betreuung · Projektführung · Datenerhebung ·
Statistik · Regulatorik · Co-Autorenschaft · Koordination

## 4. ProjectPeople

`__pkProjectPersonID` T · `_fkProjectID` T · `_fkPersonID` T · `_fkRoleID` T ·
`RACI` T (WL-13) · `Hauptverantwortlich` Z · `ErstelltAm` ZS

---

## 5. SectionTemplates — Wissensschicht, Abschnittsebene

*Wird komplett per Import gefüllt. Nach dem Import nur Typen korrigieren.*
...
`SectionNr` Z · `Nr` Z · `Name` T · `Zweck` T ·
`Stolpersteine` T · `Hinweis_Forschende` T · `Hinweis_Leitung` T

## 7. ChecklistTemplates

`SectionNr` Z · `StepNr` Z · `Nr` Z · `Punkt` T ·
`Erklaerung` T

---

## 8. Sections — Abschnitte des konkreten Projekts

*Herkunft: Bildschirme 03, 04, 05*

| Feld | Typ | Optionen |
|---|---|---|
| `__pkSectionID` | T | Standard |
| `_fkProjectID` | T | |
| `Nr` | Z | |
...
| `Notiz` | T | |
| `c_PunkteGesamt` | ⊕ Z | F-12 |
| `c_PunkteErledigt` | ⊕ Z | F-13 |
| `c_BandStartPx` `c_BandBreitePx` | ⊕ Z | F-14 |
| `c_IstStartPx` `c_IstBreitePx` | ⊕ Z | F-15 |
...
> auskommt, was ein Tabellenauftreten weniger bedeutet.

## 9. Steps

`__pkStepID` T · `_fkSectionID` T · `_fkProjectID` T · `Nr` Z · `Name` T ·
`Zweck` T · `Stolpersteine` T · `Hinweis_Forschende` T · `Hinweis_Leitung` T ·
`Status` T (WL-05) · `TrifftNichtZu` Z (Vorgabe 0) · `Ist_Ende` D ·
`c_PunkteGesamt` ⊕Z (F-12) · `c_PunkteErledigt` ⊕Z (F-13)

> `_fkProjectID` ist bewusst redundant. Es erspart auf Bildschirm 01 eine
> Bezugsebene und macht Portalfilter einfacher — der Preis ist, dass Skript S-03
> es mitschreiben muss.

## 10. ChecklistItems

| Feld | Typ | Optionen |
|---|---|---|
| `__pkItemID` | T | Standard |
| `_fkStepID` `_fkSectionID` `_fkProjectID` | T | |
| `StepNr` | Z | denormalisiert — Sortierschlüssel des Portals |
...
| `IstUeberschrift` | Z | Vorgabe 0 — siehe Kasten unten |
| `Notiz` | T | |
| `IstOffen` | ⊕ Z | F-11 — **gespeichert**, sonst sind die Summen langsam |
| `IstGezaehlt` | ⊕ Z | F-11 — **gespeichert** |
...
> `StepNr, Nr` sortiert steht sie automatisch über ihrer Gruppe; im Layout wird
...
`__pkDeliverableID` T · `_fkProjectID` T · `_fkSectionID` T · `Titel` T ·
`Typ` T · `Owner` T · `Reviewer` T · `Faellig` D · `Status` T (WL-06) ·
`Version` T · `Freigabedatum` D · `Notiz` T · `ErstelltAm` ZS

## 12. Tasks

`__pkTaskID` T · `_fkProjectID` T · `_fkSectionID` T · `_fkStepID` T ·
`_fkDeliverableID` T · `Titel` T · `Beschreibung` T · `_fkPersonID` T ·
`Termin` D · `Prioritaet` T (WL-08) · `Status` T (WL-07, Vorgabe `offen`) ·
`Aufwand_geschaetzt` Z · `Aufwand_tatsaechlich` Z · `ErstelltAm` ZS ·
`ErstelltVon` T · `c_IstOffen` ⊕Z (F-18)

> Alle vier Fremdschlüssel dürfen leer sein. Eine Aufgabe kann an einem Schritt,
> an einem Abschnitt, an einem Ergebnis oder direkt am Projekt hängen — wer ohne
> Struktur arbeiten will, kann das.

## 13. Documents

`__pkDocumentID` T · `_fkProjectID` T · `_fkSectionID` T · `_fkDeliverableID` T ·
`Titel` T · `Dokumenttyp` T (WL-12) · `AktuelleVersion` T · `Status` T (WL-06) ·
`Notiz` T · `ErstelltAm` ZS · `ErstelltVon` T

## 14. DocumentVersions

`__pkVersionID` T · `_fkDocumentID` T · `Version` T · `Datei` ⊕**C** ·
`Bemerkung` T · `HochgeladenAm` ZS · `HochgeladenVon` T
...
> Datei ins Unhandliche.

## 15. Risks

`__pkRiskID` T · `_fkProjectID` T · `_fkSectionID` T · `Kategorie` T (WL-09) ·
`Beschreibung` T · `Ursache` T · `Wahrscheinlichkeit` T (WL-14) ·
`Auswirkung` T (WL-14) · `Gegenmassnahme` T · `_fkPersonID` T · `ReviewDatum` D ·
`Status` T (WL-15) · `ErstelltAm` ZS

## 16. Manuscripts

`__pkManuscriptID` T · `_fkProjectID` T · `Arbeitstitel` T · `Zieljournal` T ·
`Alternativjournale` T · `Autorenreihenfolge` T · `Status` T (WL-10) ·
`AbschnittStatus` T · `Notiz` T · `ErstelltAm` ZS

## 17. Submissions

`__pkSubmissionID` T · `_fkManuscriptID` T · `_fkProjectID` T · `Journal` T ·
`Runde` Z · `Eingereicht_am` D · `Entscheidung` T (WL-10) · `Entscheidung_am` D ·
`Reviewerkommentare` T · `Rebuttal` T

---

## 18. Nudges — die Hinweisschicht

*Herkunft: Bildschirme 01 (Spalte rechts), 02 (unterer Bereich)*

| Feld | Typ | Optionen |
|---|---|---|
| `__pkNudgeID` | T | Standard |
| `_fkProjectID` | T | |
| `Schluessel` | T | eindeutige Kennung der Regel, z. B. `ETHIK_FEHLT` — verhindert Doppelanlage |
...
> `Schluessel` + `_fkProjectID` + `Stufe` sind zusammen eindeutig. Skript S-05
...
## 19. ActivityLog — Verlauf

`__pkActivityID` T · `_fkProjectID` T · `Zeitstempel` ZS (Auto Erstellung) ·
...
## 20. zz_Utility — Einstellungen

Genau **ein** Datensatz. Nach dem Import einmal anlegen und füllen.

`__pkUtilityID` T · `StilleFristTage` Z (Vorgabe 42) · `RuhtFristTage` Z
...
| WL-06 | `Ergebnisstatus` | offen · in Arbeit · im Review · freigegeben |
...
| WL-10 | `Publikationsstatus` | Draft · Internes Review · Submitted · Minor Revision · Major Revision · Resubmitted · Accepted · Rejected · Published |
...
| WL-13 | `RACI` | Responsible · Accountable · Consulted · Informed |
...
**Auf Zahl:** alle `Nr`, `LfdNr`, `SectionNr`, `StepNr`, `Runde`, `Rang`,
`*_Monat`, `*Monate`, `*Tage`, `*Px`, `Erledigt`, `TrifftNichtZu`,
`Weggeklickt`, `Zurueckgezogen`, `Aktiv`, `Hauptverantwortlich`,
`WochenimpulsAktiv`, `Aufwand_*`, alle `s_`-Zahlenfelder

**Auf Datum:** `Startdatum`, alle `Soll_*`, `Ist_*`, `Faellig`, `Termin`,
`ReviewDatum`, `Freigabedatum`, `Eingereicht_am`, `Entscheidung_am`,
